...
echo 'UserParameter=custom_python_script[*], python3 /etc/zabbix/test_python_script.py $1 $2' > /etc/zabbix/zabbix_agentd.d/test_user_parameter.conf
...
then
	...
	echo 'current_date = datetime.datetime.now().strftime('%d.%m.%Y')' >> /etc/zabbix/test_python_script.py
	echo 'if (sys.argv[1] == '1'): # Если 1' >> /etc/zabbix/test_python_script.py
	echo '    print(f"Сергеев А.Н.") # Выводим ФИО' >> /etc/zabbix/test_python_script.py
	echo 'elif (sys.argv[1] == '2'): # Если 2' >> /etc/zabbix/test_python_script.py
	echo '    print(current_date) # Выводим текущую дату' >> /etc/zabbix/test_python_script.py
